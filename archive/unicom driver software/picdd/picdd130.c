/*****************************************************************************
   ...
   File: PICDD.C              Version 1.3                 Date: 1/11/98   
...
newsio.c_cc[VTIME] = 1; //wait up to .1 second
...
if (strcmp(cmd,"nop") == 0)
   ...
   {
   printf("\nerror: %s extracted from %s invalid command",cmd,message);
   fprintf(diags,"\nerror: %s extracted from %s invalid command",cmd,message);
   return -2;
   }
return retval;
