/*****************************************************************************
   ...
   File: ACTOR1.C              Version 1.32                 Date: 5/20/98   
   ...
   This is the actor for UNICOM
	...
	This is a CLIENT, Talking to multiple servers
      ...
      Intended for use with the following:
		...
		Compile with:
				gcc actor132.c -o actor132 -lgdbm
      ...
      The revision history is shown below:
            ...
            Rev 1.20 3/10/98 Changed test evaluation to internal function 
				...
				Rev 1.31 5/11/98 Bug fix to remove trailing spaces left over on 
										host port commands.
				Rev 1.32 5/20/98 Increased cmd_out string lengths to 120
...
#define VERSION "Version 1.32, 5/20/98"
...
*
* 5/20/98 - Increase string lenghts to 120 bytes
**********************************************************************/
int cmd_out(int k)
{
...
memcpy(string,dbdata.dptr,dbdata.dsize);
string[dbdata.dsize]='\0';
if (strlen(string) > 119)
   {
   /* if string too long, this is an error */
   sprintf(msgstr,"string %s greater than 120 bytes\n",string);
   msg(msgstr);
   return (-2);
   }
free(dbdata.dptr);
