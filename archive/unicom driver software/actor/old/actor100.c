/*****************************************************************************
   ...
   File: ACTOR1.C              Version 1.0                 Date: 2/11/98   
   ...
   TODO:    translate test string variables
            include %internal variables
            make set variables work
            improve string processing on shell and host/port/command 
...
#define VERSION "Version 1.0, 2/9/98"
...
while(life >= 0)
   ...
   for (n=0; n<100; n++)
      {
      retval = read(insockfd, &in_string[n], 1);
      if ((in_string[n] == '\n')||(in_string[n]==0x0d)||(in_string[n]==0x0a))
         {
         in_string[n] = '\0';
         break;
         }
...
/* cmd_out - get and process a single command out line */
int cmd_out(int k)
{
...
/* make a unique name for the database */
