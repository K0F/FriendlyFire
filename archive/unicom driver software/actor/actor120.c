/*****************************************************************************
   ...
   File: ACTOR1.C              Version 1.2                 Date: 3/10/98   
   ...
   This is the actor for UNICOM
	...
	This is a CLIENT, Talking to multiple servers
      ...
      Intended for use with the following:
		...
		Compile with:
				gcc actor120.c -o actor120 -lgdbm
      ...
      The revision history is shown below:
            ...
            Rev 1.1  3/5/98  Added ability to do var lookup and limited math in
...
#define VERSION "Version 1.2, 3/10/98"
...
if (strchr(tstr,'%')!=NULL)
   ...
   while(index <= strlen(tstr))
      {
      /* first go thru and find the % variables */
      j=outdex;
      for (i=index; i<=strlen(tstr); i++)
         {
         if ((tstr[i] == '%')||(tstr[i]=='\0'))
            break;
         s2[j++]=tstr[i];
      ...
      if (strcmp(vs,"mnote") == 0)
         ...
         if (retval == -1)
            {
            sprintf(msgstr,"Unable to access logicdd in varmath\n");
            msg(msgstr);
            return -1;   /* flag as a failure */
            }
         retval = read(logicfd, rs, 79);
         if (retval < 0)
           {
           sprintf(msgstr,"Error %d reading logicdd in varmath\n",retval);
           msg(msgstr);
