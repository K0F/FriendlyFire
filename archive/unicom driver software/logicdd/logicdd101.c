/*****************************************************************************
   ...
   File: LOGICDD.C              Version 1.01                 Date: 3/2/98   
	...
	This is a SERVER, able to accept multiple clients
      ...
      The revision history is shown below:
            ...
            Rev 1.01 3/2/98 Revised to include string terminator on
                              data string during writes.
		...
		if (FD_ISSET(afd, &testfds))
			...
			if (afd == server_sockfd)
             ...
             if (retval == 2)
               ...
               dbdata.dsize = strlen(datstring)+1;
